...
double cal_eb2(double*,double*,int,int,int,int,double*,int,double,int);
double cal_eb2_par(double*,double*,int,int,int,int,double*,int,double,int);
...
//            printf("Comment line: %s",buff_line);
        ...
        dummy[param_name_len] = '\0';
        if (strcmp(dummy,param_name)==0) {
            sscanf(buff_line,"%s %lf",dummy,&eb0);
            if (rank == MASTER)
                printf("eb0 = %f\n",eb0);
        }
...
//    pr_tmp_master = (double*) calloc(pr_tmp_size,sizeof(double));
    ...
    //            cal_eb(eb,ef_tr,n1,n2,crystal1,crystal2,ef_atoms,n_crystal_type,eb0);
                cal_eb2(eb,ef_tr,n1,n2,crystal1,crystal2,ef_atoms,n_crystal_type,eb0,Natoms_instant_trans);
    //            printf("eb=(%f, %f)\n",eb[0],eb[1]);
